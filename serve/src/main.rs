...
#[tokio::main]
async fn main() {
    // Spawn job to mine ORE
    tokio::spawn(async move {
        let x = ore_miner::Miner::launch("https://ore-pool-private.onrender.com").await;
        println!("Miner launched: {:?}", x);
    });

    // Define the directory to serve files from with conditional caching
    let dir = warp::fs::dir("../dist").and(warp::path::full()).map(
        |reply, path: warp::path::FullPath| {
    ...
    let index = warp::path::end()
        ...
        .map(|reply| warp::reply::with_header(reply, "Cache-Control", "no-cache, must-revalidate"));
