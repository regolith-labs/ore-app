...
use super::use_pool_commit_claim_transaction_submit;

pub fn use_pool_register_transaction(
    mut start: Signal<bool>,
    ...
    let commit_claim_finished = use_pool_commit_claim_transaction_submit(
        pool_deprecated,
    ...
    use_resource(move || async move {
        ...
        if *commit_claim_finished.read() {
            if let Some(pool) = pool.cloned() {
                ...
                return Ok(tx_with_priority_fee);
            }
        }
