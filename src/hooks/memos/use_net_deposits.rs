            ...
            if let Some(Ok(stake)) = *stake.read() {
                if mint == &MINT_ADDRESS { 
                    ...
                    if let Some(Ok(liquidity_pair)) = liquidity_pair_resource.cloned() {
                        ...
                        let (ore_amount_pending_f64, _token_amount_pending_f64, _token_ticker_pending, _token_decimals_pending) = liquidity_pair.get_stake_amounts(stake.balance_pending);
                        let ore_amount_u64 = ui_amount_to_amount(ore_amount_f64, TOKEN_DECIMALS);
                        let ore_amount_pending_u64 = ui_amount_to_amount(ore_amount_pending_f64, TOKEN_DECIMALS);
