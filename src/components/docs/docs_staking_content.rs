...
fn StakingHowItWorks() -> Element {
    rsx! {
        ...
        BulletPointList {
            BulletPoint {
                "ORE boosts allow a governance community to issue incentives for growth initiatives such as marketing, development, liquidity, and more."
            }
            BulletPoint {
                "Yield for boosts is generated by taking a slice of all new tokens produced by miners. These incentives come from the existing emissions schedule, and do not produce any additional inflation for holders."
            }
            BulletPoint {
                "A number of boosts are currently targeted at liquidity providers to bootstrap liquidity for the token. LPs can earn boost yield by supporting ORE markets and strengthening the currency."
            }
        }
    }
}
